...
const statuses = {
	...
	2: errorCodes.MEMORY_LIMIT,
	3: errorCodes.PROCESSING_FAILED,
	4: errorCodes.INVALID_OPTIONS,
	5: errorCodes.WASM_OUT_OF_MEMORY,
};
...
async function encode() {
	...
	if (status !== 0) {
		...
		let code = statuses[status] ?? errorCodes.PROCESSING_FAILED;
		if (code === errorCodes.PROCESSING_FAILED && unsupportedInputPattern.test(diagnostic)) {
		...
		const message = code === errorCodes.WASM_OUT_OF_MEMORY
			...
			: bridgeMessage || 'Guetzli encoding failed';
		postFailure(code, message, diagnostic);
		return;
	}
